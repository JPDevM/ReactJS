...
function TodoComponent() {
	...
	if (localStorage.getItem('tareas')) {
		tareasEnStorage = JSON.parse(localStorage.getItem('tareas'));
	}

	const [todosArray, setTodosArray] = useState(tareasEnStorage);
	...
	*/
	...
	const handlerCheck = (e) => {
		let idTarea = Number(e.currentTarget.getAttribute('data-todo-id'));
		let tareasActualizadas = todosArray.map(unaTarea => {
		...
		setTodosArray(tareasActualizadas);
	}

	const handlerStorage = () => {
		localStorage.setItem('tareas', JSON.stringify(todosArray));
	}
	...
	return (
		<>
			...
			<button onClick={handlerStorage}>Guardar en Storage las tareas</button>
